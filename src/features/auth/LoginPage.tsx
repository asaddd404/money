...
export const LoginPage = () => {
  ...
  const loginMutation = useMutation({
    mutationFn: async (values: FormData) => {
      const tokens = (await http.post<Tokens>(endpoints.auth.login, values)).data;
      ...
      setSession({ accessToken: tokens.access_token, refreshToken: tokens.refresh_token, user: me });
      return me;
    },
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit((values) => loginMutation.mutate(values))}>
      ...
      <p className="text-sm text-muted-foreground">Use role credentials to access dedicated workspace.</p>
      <div>
