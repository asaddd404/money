...
export const RegisterPage = () => {
  ...
  const registerMutation = useMutation({
    mutationFn: (payload: RegisterForm) => http.post(endpoints.auth.registerStudent, payload),
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit((values) => registerMutation.mutate(values))}>
      ...
      <p className="text-sm text-muted-foreground">После регистрации войдите в аккаунт на странице логина.</p>
      ...
      <div>
        <Input type="number" placeholder="Center ID" {...register('center_id')} />
        {errors.center_id && <p className="mt-1 text-xs text-destructive">{errors.center_id.message}</p>}
      </div>
