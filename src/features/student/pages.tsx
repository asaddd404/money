...
export const StudentHomePage = () => {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Student Home" />
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl border p-4">
          Available: {data?.available ?? 0}
        </div>
        <div className="rounded-xl border p-4">Held: {data?.held ?? 0}</div>
      </div>
    </div>
  );
};
...
export const StudentWalletPage = () => {
  const { data, isLoading, error } = useQuery<WalletSummary>({
    queryKey: ['wallet'],
    queryFn: async () => (await http.get('/students/me/wallet')).data
  });
  if (isLoading) return <LoadingSkeleton />;
...
export const StudentTransactionsPage = () => {
  ...
  const { data, isLoading } = useQuery<{ items: TransactionItem[] }>({
    queryKey: ['tx', offset],
    queryFn: async () =>
      (
        await http.get('/students/me/transactions', {
  ...
  return (
    <div className="space-y-3">
      ...
      {(data?.items ?? []).map((tx) => (
        <div className="rounded-xl border p-3" key={tx.id}>
          {tx.reason} — {tx.amount}
        </div>
      ))}
      <Button variant="outline" onClick={() => setOffset((p) => p + 20)}>
        Load more
      </Button>
    </div>
  );
...
export const StudentGroupsPage = () => {
  const { data } = useQuery<{ items: GroupItem[] }>({
    queryKey: ['groups'],
    queryFn: async () =>
      (await http.get(endpoints.groups, { params: { limit: 20, offset: 0 } }))
        .data
  });
  const enrollMutation = useMutation({
    mutationFn: (groupId: string) =>
      postWithIdempotency(`${endpoints.groups}/${groupId}/enroll`, {})
  });
  return (
    <div className="space-y-3">
      ...
      {(data?.items ?? []).map((g) => (
        <div key={g.id} className="rounded-xl border p-4">
          ...
          <Button
            ...
            onClick={() => enrollMutation.mutate(g.id)}
          >
            {enrollMutation.isPending ? 'Pending...' : 'Enroll request'}
          </Button>
        </div>
      ))}
    </div>
    ...
    <div className="space-y-3">
      ...
      {(data?.items ?? []).map((p) => (
        <ProductCard
          key={p.id}
          product={p}
          onAction={() => createOrder.mutate({ product_id: p.id, quantity: 1 })}
          actionLabel={createOrder.isPending ? 'Pending...' : 'Create order'}
        />
      ))}
      <Button variant="outline" onClick={() => setOffset((p) => p + 20)}>
        Load more
...
export const StudentOrdersPage = () => {
  ...
  const cancel = useMutation({
    mutationFn: (id: string) => http.post(`${endpoints.orders}/${id}/cancel`)
  });
  ...
  if (!orders.length)
    return (
      <EmptyState
        title="No orders yet"
        description="Purchase products from shop"
      />
    );
  return (
    <div className="space-y-3">
      ...
      {orders.map((order) => (
        <OrderCard
          key={order.id}
          order={order}
          onAction={() => cancel.mutate(order.id)}
        />
      ))}
...
export const StudentLeaderboardsPage = () => {
  ...
  const { data } = useQuery<{ items: LeaderboardItem[] }>({
    queryKey: ['leaderboard', period],
    queryFn: async () =>
      (
        await http.get(`${endpoints.leaderboards}/global`, {
          params: { period }
        })
      ).data
  });
