...
async def bootstrap_admin(
    ...
    svc = AuthService(UserRepository(db), WalletRepository(db), RefreshTokenRepository(db), CenterRepository(db))
    user, center = await svc.bootstrap_admin(payload.email, payload.password, payload.full_name, payload.center_name, payload.timezone)
    await db.commit()
    return {'id': user.id, 'email': user.email, 'role': user.role.value, 'center_id': center.id, 'center_name': center.name}
