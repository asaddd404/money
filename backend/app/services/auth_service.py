...
from app.repositories.centers import CenterRepository
...
class AuthService:
    def __init__(
        ...
        centers: CenterRepository | None = None,
    ):
        self.users = users
    ...
    async def register_student(self, email: str, password: str, full_name: str, center_id: int):
        ...
        if self.centers and not await self.centers.by_id(center_id):
    ...
    async def bootstrap_admin(self, email: str, password: str, full_name: str, center_name: str, timezone: str):
        if not self.centers:
            raise HTTPException(500, 'center repository is required')
        if await self.users.by_email(email):
            raise HTTPException(409, 'email already used')
        center = await self.centers.by_name(center_name)
        if not center:
            center = await self.centers.create(name=center_name, timezone=timezone)
        user = await self.users.create(
            email=email,
            password_hash=hash_password(password),
            full_name=full_name,
    ...
    async def login(self, email: str, password: str, access_minutes: int, refresh_days: int):
        ...
        await self.refresh_repo.create(
            user_id=u.id,
            jti=payload['jti'],
            token_hash=refresh_hash(refresh),
    ...
    async def refresh(self, refresh_token: str, access_minutes: int, refresh_days: int):
        ...
        await self.refresh_repo.create(
            ...
            jti=new_payload['jti'],
            token_hash=refresh_hash(new_refresh),
            expires_at=now_utc() + timedelta(days=refresh_days),
        )
