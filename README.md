...
## Быстрый способ обновить PR-ветку

```bash
bash scripts/sync-with-main.sh
```

Скрипт сделает `fetch + merge` из `origin/main`, проверит conflict-маркеры и подскажет следующие команды для проверки и push.
...
Дальше откройте конфликтные файлы, оставьте нужный код и удалите служебные строки разметки конфликта.
